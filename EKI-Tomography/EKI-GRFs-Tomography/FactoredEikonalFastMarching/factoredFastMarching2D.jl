function factoredFastMarching(pEik::EikonalParam, mem::EikonalTempMemory)
	...
	src_cs = loc2cs(src,n);	# src_cs is the offset of the origin

	kappaSrc = sqrt(kappaSquared[src_cs]);
	T[src_cs] = kappaSrc;	# τ1[origin]
	...
	while frontHeap.size > 0
		# tic();
		curr = getMin(frontHeap,curr_val); # offset
		cs2loc(curr_loc,curr,n);	# coordinate
