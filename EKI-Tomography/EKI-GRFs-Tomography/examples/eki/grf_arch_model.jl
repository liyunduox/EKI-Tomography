...
# set output dir
homedir = pwd()
figure_save_directory = homedir * "/output_arch/"    # output results
data_save_directory = homedir * "/output_arch/"
...
@everywhere function subsurface_receiver_samples_and_vcat(τ_array::Vector{Matrix{Float64}}, count::Int64)
    n = length(τ_array)
    step = 2 # div(size(τ_array[1])[2], count)
    last = size(τ_array[1])[2] # step * count
    data_length = 121
    # receiver_sample(τ, s, last) = [τ[(i - 1) * size(τ)[1] + 1] for i in s:s:last]
    receiver_sample(τ, s, last) = [τ[1, i] for i in 1:s:last]
    # receiver_sample(τ, s, last) = [τ[i * size(τ)[1]] for i in s:s:last]
    samples = zeros(data_length, n)
    for i in 1:n
        samples[:, i] = receiver_sample(τ_array[i], step, last)
    end
    reduce(vcat, samples)
end
...
function run()
    ...
    vel_origin = generate_velocity2()
    vel_sampling_interval = 1
    vel = vel_origin[1:vel_sampling_interval:end, 1:vel_sampling_interval:end]
    I = [1.0, 4.8]; tmp_index = 13;
    fig_rows = 3; fig_cols = 1;  single_line_target_col1 = 40; single_line_target_col2 = 80; single_line_target_col3 = 121; single_line_target_col4 = 161
    fig_size = [15, 10]; subplot_title_height = - 0.32; legend_height = - 0.32;  
    up_bounded_global = 4.32; low_bounded_global = 2.0; xticks_num = 9; 
    recv_marker_size = 11; recv_marker_itvl = 3; src_marker_size = 8; src_marker_itvl = 10; src_marker_offset = 0.5;
    ...
    # subsurface
    N_receivers = n[2]
    src = [[1, i] for i in 1:10:n[2]] # v2: 10
    receiver_samples_and_vcat = subsurface_receiver_samples_and_vcat
