...
@everywhere function crosswell_receiver_samples_and_vcat2(τ_array::Vector{Matrix{Float64}}, count::Int64)
    n = length(τ_array)
    # step = div(size(τ_array[1])[1], count)
    # last = step * count
    step1 = 2
...
function run()
    ...
    println("******************** Velocity Model ***************************");
     ...
     # overthrust
     vel_origin = read_model_from_mat_transpose("D:/arch/data/model/overthrust.mat", "y400"; velscale = 0.001)
    ...
    N_receivers = n[1]
    src = []
    for i in 1:20:n[2]  # overthrust: 15  v1: 5
        push!(src, [1, i])
    end
